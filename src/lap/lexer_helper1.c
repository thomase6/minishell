/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   lexer_helper1.c                                    :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: texenber <texenber@student.42vienna.com    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/03/10 09:25:02 by stbagdah          #+#    #+#             */
/*   Updated: 2026/04/22 12:24:53 by stbagdah         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../inc/lap.h"

int	scan_single_quote(t_shell *shell, t_scan_ctx *cntx, int i)
{
	int		start;
	t_token	*tok;

	start = i + 1;
	i = start;
	while (cntx->input[i] && cntx->input[i] != '\'')
		i++;
	if (!cntx->input[i])
	{
		handle_syntax_error(shell, "'", 0);
		return (-1);
	}
	tok = add_token(cntx->head, (t_token_data){TOKEN_WORD,
			&cntx->input[start], i - start, cntx->has_space});
	if (!tok)
		return (-1);
	tok->quoted = 1;
	return (i + 1);
}

int	scan_double_quote(t_shell *shell, t_scan_ctx *cntx, int i)
{
	int		start;
	t_token	*tok;

	start = i + 1;
	i = start;
	while (cntx->input[i] && cntx->input[i] != '"')
		i++;
	if (!cntx->input[i])
	{
		handle_syntax_error(shell, "\"", 0);
		return (-1);
	}
	tok = add_token(cntx->head, (t_token_data){TOKEN_WORD,
			&cntx->input[start], i - start, cntx->has_space});
	if (!tok)
		return (-1);
	tok->quoted = 2;
	return (i + 1);
}
